...
class CRUDChallenge(CRUDBase[Challenge, ChallengeCreate, ChallengeUpdate]):
    ...
    def read(self, db: Session, challenger_email: str, challengee_email: str, quest_name: str, category_name: str) -> List[Challenge]:
        results = db.query(models.Challenge)

        if challenger_email is not None:
            results = results.filter(models.Challenge.challenger_email == challenger_email)

        if challengee_email is not None:
            results = results.filter(models.Challenge.challengee_email == challengee_email)
